...
while ($run -eq 0) {
    ...
    $color_data += 1
    $first = 0
    foreach($file in (Get-ChildItem $dir -Exclude *.ps1, *.txt, *.pdf)) { #this part turns each file into a csv and then gets the data from it
    ...
    $outputfile = $dir + "\" + $store + "_" + "results.xls"
